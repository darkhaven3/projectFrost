    ...
    /*
		if (loadmodel->bspversion == HL_BSPVERSION) 
		{
			...
			if (COM_CheckParm("-nearest")){
				...
				{
					extern unsigned char*	hlbsp_texpalette;
					tx->gl_texturenum = GL_LoadPalettedTexture (mt->name, tx->width, tx->height, (byte *)data, qtrue, GU_NEAREST, level, hlbsp_texpalette);
					mapTextureNameList.push_back(tx->gl_texturenum);
	
					free(hlbsp_texpalette);
					free(data);
					continue;
				}
				else
				{
					extern unsigned char*	hlbsp_texpalette;
					tx->gl_texturenum = GL_LoadPalettedTexture (mt->name, tx->width, tx->height, (byte *)data, qtrue, GU_LINEAR, level, hlbsp_texpalette);
					mapTextureNameList.push_back(tx->gl_texturenum);
...
=================
*/
static void Mod_LoadVertexes (lump_t *l, int storebounds)
{
	dvertex_t	*in;
	mvertex_t	*out;
	int			i, count;
...
#define MAX_SURF_EXTENTS 2000 // Kurok is using 2000
/*
================
CalcSurfaceExtents
...
void Mod_LoadBrushModel (model_t *mod, void *buffer)
{
...

	{
		...
		loadmodel->isworldmodel = !strcmp (loadmodel->name, va("maps/%s.bsp", host_mapname.string)) ? qtrue:qfalse;
	}
// swap all the lumps
...
void * Mod_LoadSpriteGroup (void * pin, mspriteframe_t **ppframe, int framenum)
{
	...
	pspritegroup = static_cast<mspritegroup_t*>(Hunk_AllocName (sizeof (mspritegroup_t) + (numframes - 1) * sizeof (pspritegroup->frames[0]), loadname));
...
================
*/
extern "C" void Mod_Print_f (void)
{
	int		i;
	model_t	*mod;
